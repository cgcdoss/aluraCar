  ...
  agenda() {
    if (!this.checaObrigatorios()) {
      ...
      this._service
        ...
        .then(confirmado => {
          confirmado ? this.alerta.setSubTitle('Agendamento realizado com sucesso') :
            this.alerta.setSubTitle('Não foi possível realizar o agendamento');
          this.alerta.present();
        })
        .catch(err => {
          console.log(err);
          this.alerta.setSubTitle(err.message).present();
      })
    }
  }
